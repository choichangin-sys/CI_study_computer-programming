...
BOOL AddSocketInfo(SOCKET sock)
{
	...
	ZeroMemory(&(ptr->overlapped), sizeof(ptr->overlapped)); // overlapped 초기화.
	ptr->overlapped.hEvent = hEvent; // overlapped가 가지고 있는 이벤트만 입력. (이벤트 객체)
	ptr->sock = sock; // 소켓 등록
	ptr->recvbytes = 0; 
	ptr->sendbytes = 0;
	ptr->wsabuf.buf = ptr->buf; // 버퍼의 주소를 wsabuf에 저장.
								// 실직적 데이터는 buf에 들어가고 나옴.
...
unsigned int WINAPI WorkerThread(void* pParam)
{
	...
	while (1) {
		// 이벤트 객체 관찰
		DWORD index = WSAWaitForMultipleEvents(nTotalSockets, // 무한정 대기
