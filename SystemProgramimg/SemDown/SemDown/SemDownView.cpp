...
DWORD WINAPI ThreadDownLoad(LPVOID p)
{
	...
	/*
	최대자원 = 세마포어가 제어할 수 있는 리소스의 최대 개수
	사용자원 개수 = 사용 중인 리소스의 개수
	유효자원 개수 = 현재 사용할 수 있는 리소스의 개수
	*/

	pDC->TextOut(10, y, _T("다운로드가 완료되었습니다."));
	pView->ReleaseDC(pDC);
	return 0;
}
...
void CSemDownView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	DWORD dwID;
	CloseHandle(CreateThread(NULL, 0, ThreadDownLoad, this, 0, &dwID));
