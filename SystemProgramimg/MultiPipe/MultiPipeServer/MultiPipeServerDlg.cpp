...
DWORD WINAPI ClientThread(LPVOID p)
{
	...
	for (;;) // 하나의 고객을 위한 서비스를 무한히 제공하는 for
	{
		// 1. 파이프로 부터 문자 읽기
		bSuccess = ReadFile(hPipe, szInput, 255, &dwRead, NULL);

		if ((bSuccess == FALSE) || (bSuccess == 0))
		{
			break;
		}
		...
		// 3. 파이프에 문자 쓰기
		WriteFile(hPipe, szOutput, (lstrlen(szOutput) + 1) * sizeof(TCHAR),
				&dwWritten, NULL);
...
DWORD WINAPI ListenThread(LPVOID p)
{
	for (;;) // 여러 명의 고객을 접속시키기 위한 for
	{
		...
		{
			// 3. 번역 서비스 진행
			CloseHandle(CreateThread(NULL, 0, ClientThread,
									(LPVOID)hPipe, 0, 0));
		}
		else
		{
