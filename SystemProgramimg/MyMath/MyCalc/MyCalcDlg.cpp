...
// 명시적 연결 ///////////////////////////
// 실행 중간 : 명시적 연결
// dll이 없으면 기능 사용 불가, 켜지긴 함.
void CMyCalcDlg::OnBnClickedButton2()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	// 1. DLL을 로드한다. (생성자 가능)
	HINSTANCE hDLL;
	hDLL = LoadLibrary(_T("MyMathLib.dll"));
	...
	// 2. 그 DLL에서 원하는 함수를 찾는다.
	//int Add(int a, int b);
	int (*pFunc)(int , int ) = NULL; // 위의 함수를 가리키는 함수포인터
	pFunc =(int (*)(int, int))GetProcAddress(hDLL, ("Sub")); // 함수포인터로 강제 형변환
	if (pFunc == NULL)
	{
