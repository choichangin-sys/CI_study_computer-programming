...
{
	g_dwTlsIndex = TlsAlloc(); // 시작, DWORD로 선언
...
void CBackThreadDlg::OnBnClickedButton2()
{
	...
	TlsFree(g_dwTlsIndex); // 종료, DWORD로 선언
}
...
/*
	스레드에서 전역변수, 정적변수를 참조하면 위험하다. (경쟁상태가 됨)
*/
...
int GetAddNum()
{
	...
	// TLS(Thread Local Storage)로 설정은 야매임. (MS에서만 사용 가능한 방식임. __declspec(thread))
...
{
	int* arrWork = (int*)p;
	// arrWork의 데이터를 가공한다. (정렬, 통계 등)

	for (int i = 0; i < 100; i++)
	{
		arrWork[i] = arrWork[i] * 2;
		arrWork[i] = arrWork[i] + GetAddNum();
		Sleep(50); // 정렬, 통계 등의 복잡한 작업을 가정함.
	}
	// 계산 완료된 데이터를 파일에 저장한다.
	CString strFileName;
	strFileName.Format(_T("c:\\temp\\Result%d.txt"),
						++nFileIndex);
...
void CBackThreadDlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	if (nIDEvent == 1)
	{
		...
		if (nIndex == 100)
		{
			...
			// 작업 복사본을 만든다.
			int* arrCopy = (int*)malloc(sizeof(arrData)); // 데이터 잡아주고
			memcpy(arrCopy, arrData, sizeof(arrData)); // 복사

			DWORD ID;
			CloseHandle(CreateThread(NULL, 0, CalcThread, arrCopy, 0, &ID));
			//CloseHandle(CreateThread(NULL, 0, CalcThread, arrData, 0, &ID));
