...
// CNamedPipeClientDlg 대화 상자



CNamedPipeClientDlg::CNamedPipeClientDlg(CWnd* pParent /*=nullptr*/)
...
BEGIN_MESSAGE_MAP(CNamedPipeClientDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON1, &CNamedPipeClientDlg::OnBnClickedButton1)
END_MESSAGE_MAP()


// CNamedPipeClientDlg 메시지 처리기
...
{
	CNamedPipeClientDlg* pWnd = (CNamedPipeClientDlg*)p;
	HANDLE hPipe;
	for (;;)
	{
	...
	pWnd->SetDlgItemText(IDC_EDIT1, buf);
...
void CNamedPipeClientDlg::OnBnClickedButton1()
{
	DWORD ID;
	CloseHandle(CreateThread(NULL, 0, ClientThread, this, 0, &ID));
}
