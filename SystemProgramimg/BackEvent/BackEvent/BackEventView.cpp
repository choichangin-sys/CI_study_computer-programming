...
CBackEventView::CBackEventView() noexcept
{
	...
	hEvent30 = CreateEvent(NULL,
		FALSE, // FALSE - 자동리셋, TRUE - 수동리셋
		FALSE,
		NULL);
	hEventCalc = CreateEvent(NULL,
		TRUE, // FALSE - 자동리셋, TRUE - 수동리셋
		FALSE,
		NULL);
}
...
void CBackEventView::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	//CloseHandle(CreateThread(NULL, 0, ThreadFunc, 0, 0, &dwID));
	...
	for (int i = 0; i <= 30; i++)
	{
		strTemp.Format(_T("%d 라인은 %d입니다."), i, buf[i]);
		pDC->TextOut(10, i * 20, strTemp);
	}
...
DWORD WINAPI ThreadFSend(LPVOID p)
{
	WaitForSingleObject(hEventCalc, INFINITE);
...
void CBackEventView::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	hEventCalc = CreateThread(NULL, 0, ThreadCalc, this, 0, &dwID);
	//CloseHandle(CreateThread(NULL, 0, ThreadCalc, 0, 0, &dwID));

	CloseHandle(CreateThread(NULL, 0, ThreadSave, this, 0, &dwID));
	CloseHandle(CreateThread(NULL, 0, ThreadFSend, this, 0, &dwID));
