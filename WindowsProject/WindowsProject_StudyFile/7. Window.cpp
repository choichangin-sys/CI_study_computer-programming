...
{
    static HBRUSH RGB_RED;
    static HBRUSH RGB_BLUE;
    static HBRUSH RGB_GREEN;
    static HBRUSH NowBrush;
    switch (message)
    {
    case WM_CREATE:
    {
        RGB_RED   = CreateSolidBrush(RGB(255, 0, 0));
        RGB_BLUE  = CreateSolidBrush(RGB(0, 255, 0));
        RGB_GREEN = CreateSolidBrush(RGB(0, 0, 255));
        NowBrush = RGB_RED;
    }
    break;
    case WM_KEYDOWN:
    ...
    case WM_CHAR: //키보드에서 문자 키가 눌린 때
    {
        if ((TCHAR)wParam == 'n')
        {
            ...
            {
                MessageBox(hWnd, _T("메모장을 먼저 실행해 주세요."), _T("알림"), MB_OK);
            }
        }
    }
    break;
    case WM_LBUTTONDOWN:
    {
        SetWindowPos(hWnd, HWND_TOPMOST, 100, 100, 500, 500,
            SWP_NOMOVE | SWP_NOSIZE);
    }
    break;
    case WM_RBUTTONDOWN:
    {
        SetWindowPos(hWnd, HWND_NOTOPMOST, 100, 100, 500, 500,
            SWP_NOMOVE | SWP_NOSIZE);
    }
    break;
    case WM_PAINT:
