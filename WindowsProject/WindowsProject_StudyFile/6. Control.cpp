...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_LBUTTONDOWN:
        ...
        SetWindowText(hBtn_Save, _T("역마살 낀 듯."));
        break;
    case WM_RBUTTONDOWN:
        SetParent(hBtn_Save, hWnd);
        SetWindowText(hBtn_Save, _T("각설이 마냥 돌아옴."));
        break;
    case WM_KEYDOWN:
    {
        switch (wParam)
        ...
        case VK_LEFT:
            ...
            if (bShow == TRUE)
            {
                ...
                SetWindowText(hWnd, _T("사라졌어"));
                SetWindowText(hBtn_Save, _T("사라졌어"));
            }
            else
            {
                bShow = TRUE;
                ShowWindow(hEdit, SW_SHOW);
                SetWindowText(hWnd, _T("나타났어"));
                SetWindowText(hBtn_Save, _T("나타났어"));
            }
            break;
        }
    ...
    break;
    case WM_COMMAND: // 명령을 접수 하는 곳.
    {
        switch (LOWORD(wParam))
        {
        case ID_EDIT_SAVE:
        {
            MessageBox(hWnd, TEXT("저장를 시작합니다."),
                             TEXT("Saving"), MB_OK);
        }
        break;
        case ID_EDIT_PRINT:
        {
            MessageBox(hWnd, TEXT("인쇄를 시작합니다."),
                             TEXT("Printing"), MB_OK);
        }
        break;
        case ID_EDIT_NAME:
