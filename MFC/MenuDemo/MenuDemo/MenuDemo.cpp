...
void CMenuDemoApp::OnMusicStop()
{
	...
	AfxMessageBox(_T("음악을 정지합니다."));
	m_bPlay = FALSE;
}


void CMenuDemoApp::OnUpdateMusicStart(CCmdUI* pCmdUI)
{
	if (m_bPlay) // 재생중인가?
	{
		// 필요 없음.
...
void CMenuDemoApp::OnUpdateMusicStop(CCmdUI* pCmdUI)
{
...
void CMenuDemoApp::OnMusicPause()
{
	...
	{
		// 다시 재생 하겠다.
		AfxMessageBox(_T("다시 재생합니다."));
		m_bPause = FALSE;
	}
	else
	{
		// 일시정지 하겠다.
		AfxMessageBox(_T("일시 정지를 합니다."));
		m_bPause = TRUE;
	}
}
...
void CMenuDemoApp::OnUpdateMusicPause(CCmdUI* pCmdUI)
{
	...
	{
		// 다시 재생
		pCmdUI->SetText(_T("다시 재생")); // 글씨를 바꾸는 법
		pCmdUI->SetCheck(1); // 체크표시를 해주는 법
	}
	else
	{
