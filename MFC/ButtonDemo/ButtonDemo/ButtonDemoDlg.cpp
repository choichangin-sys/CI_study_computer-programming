...
void CButtonDemoDlg::OnBnClickedStartV3()
{
	...
	if (m_bAll) { AfxMessageBox(_T("전체 영역 검사를 시작합니다.")); }
	if (m_bUpdate) { AfxMessageBox(_T("업데이트 영역 검사를 시작합니다.")); }
	if (m_bSystem) { AfxMessageBox(_T("시스템 영역 검사를 시작합니다.")); }

	// 3. OS별로 검사를 진행한다.
	switch (m_nOS)
	{
	case 0:
		AfxMessageBox(_T("안드로이드"));
		break;
	case 1:
		AfxMessageBox(_T("윈도우"));
		break;
	case 2:
		AfxMessageBox(_T("리눅스"));
		break;
	}
...
void CButtonDemoDlg::OnBnClickedCheckAll()
{
	...
	if (m_bAll) // 전체 검사를 할 의향이 있다.
	{
		...
		//GetDlgItem(IDC_CHECK_UPDATE)->ShowWindow(0); // 다른 버튼을 지움.
		//GetDlgItem(IDC_CHECK_SYSTEM)->ShowWindow(0); // 다른 버튼을 지움.

		GetDlgItem(IDC_START_V3)->SetWindowText(_T("전체 검사를 하겠다."));

		this->SetWindowText(_T(" 하겠다."));
	}
	else // 전체 검사를 할 의향이 없다.
	{
		...
		//GetDlgItem(IDC_CHECK_UPDATE)->ShowWindow(1); // 다른 버튼을 지움.
		//GetDlgItem(IDC_CHECK_SYSTEM)->ShowWindow(1); // 다른 버튼을 지움.

		GetDlgItem(IDC_START_V3)->SetWindowText(_T("전체 검사를 안하겠다."));

		SetWindowText(_T("안하겠다."));
	}
}
