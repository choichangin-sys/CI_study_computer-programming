...
void CTreeDemoView::OnInitialUpdate() // OnInitDalog와 같은 것임.
{
	...
	// 2. 각 방에 이미지를 추가한다.
	CBitmap bmpTree;
	bmpTree.LoadBitmap(IDB_BITMAP_TREE);
	...
	HTREEITEM hfa, hMe;
	HTREEITEM hSfa;
	hHal = m_ctrlFamily.InsertItem(_T("할아버지"), 0, 1, TVI_ROOT);

	hBfa = m_ctrlFamily.InsertItem(_T("큰아버지"), 2, 0, hHal);
	m_ctrlFamily.InsertItem(_T("사촌 형"), 2, 0, hBfa);
	m_ctrlFamily.InsertItem(_T("사촌 누나"), 2, 0, hBfa);

	hfa = m_ctrlFamily.InsertItem(_T("아버지"), 3, 0, hHal);
	m_ctrlFamily.InsertItem(_T("형"), 3, 0, hfa);
	hMe = m_ctrlFamily.InsertItem(_T("나"), 3, 0, hfa);
	m_ctrlFamily.InsertItem(_T("동생"), 3, 0, hfa);

	hSfa = m_ctrlFamily.InsertItem(_T("작은아버지"), 4, 0, hHal);
	m_ctrlFamily.InsertItem(_T("사촌 동생"), 4, 0, hSfa);
